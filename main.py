...
from tkinter import filedialog, Frame, Tk, Label, Button
import re
...
def get_time():
    d = datetime.today()
    res = datetime.strftime(d, timeFormat)
    return d, res


# 获取字符串中十四位或12位的时间戳，并转换为时间
...
# 获取目录下所有图像， 目前支持bmp，png，jpg和jpeg
...
# 将时间秒数转换为时间（天时分秒）
def transfer_time(dtime):
    day, hour, m = 24 * 60 * 60, 60 * 60, 60
    ...
    if dtime >= m:
        mins, dtime = divmod(dtime, m)
    secs = dtime
    return "{}天{}时{}分{}秒".format(int(days), int(hours), int(mins), int(secs))


# 获取核酸结果，由于字体原因，有的图像并不能很好的识别字体，故直接分析图像RGB值，较为粗暴，存在假阴，假阳情况。
...
def process():
    ...
    for i in range(len(files)):
        result, data = paddle_ocr(ocr, files[i])
        today, ti = get_time()
        re_name = ""
        re_hesuanresult = ""
        re_jiancejigou = ""
        ...
        try:
            ttime = datetime.strptime(time, timeFormat2)
            time = datetime.strftime(ttime, timeFormat)
        except ValueError:
            ttime = datetime.strftime('202001010000', timeFormat2)
            time = datetime.strftime(ttime, timeFormat)
        ...
        worksheet.write(i + 1, 5, transfer_time((today - ttime).total_seconds()))
